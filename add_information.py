...
# Reads data 1000 rows at a time and finds relevant social data based on zip code
def update_columns(filename):

	race_data = read_and_refine_race_metrics()
	income_data = read_and_refine_income_metrics()
	education_data = read_and_refine_education_metrics()
	for chunk in pd.read_csv(filename, chunksize=1000):
		chunk = chunk.dropna()
		s = time.time()
		updated_data = add_race_metrics(chunk, race_data)
		updated_data = add_income_metrics(chunk, income_data)
		updated_data = add_education_metrics(chunk, education_data)
		e = time.time()
...
# Adds the income data to each row based on zip code
...
# Adds the education data to each row based on zip code
...
# Adds the race data to each row based on zip code
...
# Reads in income data and formats to our liking
...
# Reads in education data and formats to our liking
...
# Reads in race data and formats to our liking
...
# Main method
def main():
	df = update_columns('combined.csv')
	start = time.time()
	df.to_csv("ready_for_metrics.csv", index=False)
	end = time.time()
